...
import {
  ...
  type DeliveryChallanExportFormat,
...
export default function DeliveryChallanList({
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<DeliveryChallanListRowDto | null>(null);
  const [printBusyId, setPrintBusyId] = useState<string | null>(null);

  const { removeMutation } = useDcMutations(config);
  ...
  const { data, isPending, isError, error, refetch } = useDcList(
    config,
    params
  );
  ...
  const handleExport = async (
    ...
    try {
      await deliveryChallanService.exportFile(
        config.apiPath,
        config.title,
        {
          search,
  ...
  const handlePrint = async (
    ...
    try {
      await deliveryChallanService.printDocument(
        config.apiPath,
        row.id,
        row.docNo,
        mode
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          {getApiErrorMessage(error, `Unable to load ${config.title} records.`)}
  ...
  return (
    <>
                ...
                <tr>
                  <td colSpan={COLUMNS.length + 1}>
                    <div className="empty">
                      ...
                      No records found. Click “Add {config.title}”.
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmActionModal
        ...
        body="The delivery challan will be permanently removed from the database."
        okLabel="Delete"
        danger
