...
import type { StockAmendmentListRowDto } from '../../../../types/inventory/adjustment.types';
...
interface StockAmendmentListProps {
  onAdd: () => void;
  onEdit: (id: string) => void;
  onView: (id: string) => void;
}

export default function StockAmendmentList({
  onAdd,
  onEdit,
  onView,
}: StockAmendmentListProps) {
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<StockAmendmentListRowDto | null>(null);

  const { removeMutation } = useStockAmendmentMutations();
  ...
  const handleExport = async (format: AdjustmentExportFormat) => {
    try {
      await stockAmendmentService.exportFile(
        'Stock Amendment',
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          Loading Stock Amendment records...
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          {getApiErrorMessage(error, 'Unable to load Stock Amendment records.')}
  ...
  return (
    <>
        ...
        <div className="toolbar" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
          <div className="searchwrap" style={{ minWidth: '220px', flex: '0 1 260px' }}>
            ...
            <input
              className="in"
              value={searchInput}
              placeholder="Search Stock Amendment..."
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        ...
                        title="Download PDF"
                        onClick={() =>
                          stockAmendmentService.printDocument('/inventory/adjustment/stock-amendment', row.id, 'download')
                        }
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Print"
                        onClick={() =>
                          stockAmendmentService.printDocument('/inventory/adjustment/stock-amendment', row.id, 'print')
                        }
                      >
                ...
                <tr>
                  <td colSpan={COLUMNS.length + 1}>
                    <div className="empty">
                      ...
                      No records found. Click "Add Stock Amendment".
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmActionModal
        ...
        body="The stock amendment will be permanently removed from the database."
        okLabel="Delete"
        danger
