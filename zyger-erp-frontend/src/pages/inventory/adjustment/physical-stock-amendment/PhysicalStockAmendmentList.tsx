...
import {
  usePhysicalStockAmendmentList,
  usePhysicalStockAmendmentMutations,
} from '../../../../hooks/useAdjustment';
import {
  physicalStockAmendmentService,
  type AdjustmentExportFormat,
} from '../../../../services/adjustmentService';
import type { PhysicalStockAmendmentListRowDto } from '../../../../types/inventory/adjustment.types';
...
const COLUMNS: ColumnConfig[] = [
  ...
  { label: 'Location', field: 'storeLocation' },
  { label: 'Type', field: 'countType' },
  { label: 'Qty', field: 'qty', numeric: true },
  { label: 'Status', field: 'status' },
];

interface PhysicalStockAmendmentListProps {
  onAdd: () => void;
  onEdit: (id: string) => void;
  onView: (id: string) => void;
}

export default function PhysicalStockAmendmentList({
  onAdd,
  onEdit,
  onView,
}: PhysicalStockAmendmentListProps) {
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<PhysicalStockAmendmentListRowDto | null>(null);

  const { removeMutation } = usePhysicalStockAmendmentMutations();
  ...
  const { data, isPending, isError, error, refetch } =
    usePhysicalStockAmendmentList(params);
  ...
  const handleExport = async (format: AdjustmentExportFormat) => {
    try {
      await physicalStockAmendmentService.exportFile(
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          Loading Physical Stock Amendment records...
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          {getApiErrorMessage(
            error,
            'Unable to load Physical Stock Amendment records.'
          )}
  ...
  return (
    <>
      <div className="pg-head">
        <h1>Physical Stock Amendment</h1>
        ...
        <div className="toolbar" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
          <div className="searchwrap" style={{ minWidth: '220px', flex: '0 1 260px' }}>
            ...
            <input
              className="in"
              value={searchInput}
              placeholder="Search Physical Stock Amendment..."
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        ...
                        title="Download PDF"
                        onClick={() =>
                          physicalStockAmendmentService.printDocument('/inventory/adjustment/physical-stock-amendment', row.id, 'download')
                        }
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Print"
                        onClick={() =>
                          physicalStockAmendmentService.printDocument('/inventory/adjustment/physical-stock-amendment', row.id, 'print')
                        }
                      >
                ...
                <tr>
                  <td colSpan={COLUMNS.length + 1}>
                    <div className="empty">
                      ...
                      No records found. Click "Add Physical Stock Amendment".
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmActionModal
        ...
        body="The physical stock amendment will be permanently removed from the database."
        okLabel="Delete"
        danger
