...
export default function StockIssueList({
  ...
  const { data, isPending, isError, error, refetch } = useStockIssueList(
    config,
    params
  );
  ...
  const handleExport = async (format: StockIssueExportFormat) => {
    try {
      await stockIssueService.exportFile(
        config.apiPath,
        config.title,
        {
          search,
        ...
        <div className="toolbar" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <button className="btn btn-p" onClick={onAdd}>
              ...
              Add {config.docLabel || 'Stock Issue'}
            </button>
          </div>
        </div>
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        className="ibtn"
                        title="Download PDF"
                        onClick={() => stockIssueService.printDocument(config.apiPath, row.id, 'download')}
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        className="ibtn"
                        title="Print"
                        onClick={() => stockIssueService.printDocument(config.apiPath, row.id, 'print')}
                      >
