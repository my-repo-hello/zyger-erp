...
import type { RmiListRowDto } from '../../../../types/inventory/rmIssue.types';
...
export default function RmIssueList({ onAdd, onEdit, onView }: RmIssueListProps) {
  ...
  const { data, isPending, isError, error, refetch } = useRmIssueList(params);
  ...
  const handleExport = async (format: RmiExportFormat) => {
    try {
      await rmIssueService.exportFile(
        {
          search,
          status,
          sort: `${sortField},${sortDir}`,
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          {getApiErrorMessage(error, 'Unable to load RM Issue records.')}
  ...
  return (
    <>
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        ...
                        title="Download PDF"
                        onClick={() =>
                          stockIssueService.printDocument('/inventory/stock-issue/rm-issue', row.id, 'download')
                        }
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Print"
                        onClick={() =>
                          stockIssueService.printDocument('/inventory/stock-issue/rm-issue', row.id, 'print')
                        }
                      >
                ...
                <tr>
                  <td colSpan={COLUMNS.length + 1}>
                    <div className="empty">
                      ...
                      No records found. Click “Add RM Issue”.
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmActionModal
        ...
        body="The RM Issue will be permanently removed from the database."
        okLabel="Delete"
        danger
