...
import {
  useReturnManagementList,
  useReturnManagementMutations,
} from '../../../../hooks/useReturnManagement';
import {
  createReturnManagementService,
  type ReturnManagementExportFormat,
} from '../../../../services/returnManagementService';
import type {
...
interface ReturnManagementListProps {
...
export default function ReturnManagementList({
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<ReturnManagementListRowDto | null>(null);

  const { removeMutation } = useReturnManagementMutations(config);
  ...
  const { data, isPending, isError, error, refetch } =
    useReturnManagementList(config, params);
  ...
  const handleExport = async (format: ReturnManagementExportFormat) => {
    try {
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        className="ibtn"
                        title="Download PDF"
                        onClick={() => createReturnManagementService(config.apiPath).printDocument(row.id, 'download')}
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        className="ibtn"
                        title="Print"
                        onClick={() => createReturnManagementService(config.apiPath).printDocument(row.id, 'print')}
                      >
