...
import type { PurchaseInvoiceListRowDto } from '../../../../types/inventory/supplierInvoice.types';
...
export default function PurchaseInvoiceList({
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<PurchaseInvoiceListRowDto | null>(null);

  const { removeMutation } = usePurchaseInvoiceMutations();
  ...
  const handleExport = async (format: SupplierInvoiceExportFormat) => {
    try {
      await purchaseInvoiceService.exportFile(
        'Purchase Invoice',
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          Loading Purchase Invoice records...
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          {getApiErrorMessage(error, 'Unable to load Purchase Invoice records.')}
  ...
  return (
    <>
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        ...
                        title="Download PDF"
                        onClick={() =>
                          purchaseInvoiceService.printDocument('/inventory/supplier-invoice/purchase-invoice', row.id, 'download')
                        }
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Print"
                        onClick={() =>
                          purchaseInvoiceService.printDocument('/inventory/supplier-invoice/purchase-invoice', row.id, 'print')
                        }
                      >
                ...
                <tr>
                  <td colSpan={COLUMNS.length + 1}>
                    <div className="empty">
                      ...
                      No records found. Click "Add Purchase Invoice".
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmActionModal
        ...
        body="The purchase invoice will be permanently removed from the database."
        okLabel="Delete"
        danger
