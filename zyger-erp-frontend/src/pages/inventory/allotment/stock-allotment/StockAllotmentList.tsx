...
import {
  useStockAllotmentList,
  useStockAllotmentMutations,
} from '../../../../hooks/useAllotment';
import {
  stockAllotmentService,
  type AllotmentExportFormat,
} from '../../../../services/allotmentService';
import type { StockAllotmentListRowDto } from '../../../../types/inventory/allotment.types';
...
const COLUMNS: ColumnConfig[] = [
  ...
  { label: 'Type', field: 'allotmentType' },
  { label: 'Reference', field: 'referenceNo' },
  { label: 'Qty', field: 'qty', numeric: true },
  { label: 'Status', field: 'status' },
];

interface StockAllotmentListProps {
  onAdd: () => void;
  onEdit: (id: string) => void;
  onView: (id: string) => void;
}

export default function StockAllotmentList({
  onAdd,
  onEdit,
  onView,
}: StockAllotmentListProps) {
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<StockAllotmentListRowDto | null>(null);

  const { removeMutation } = useStockAllotmentMutations();
  ...
  const handleExport = async (format: AllotmentExportFormat) => {
    try {
      await stockAllotmentService.exportFile(
        'Stock Allotment',
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          Loading Stock Allotment records...
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          {getApiErrorMessage(error, 'Unable to load Stock Allotment records.')}
  ...
  return (
    <>
        ...
        <div className="toolbar" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
          <div className="searchwrap" style={{ minWidth: '220px', flex: '0 1 260px' }}>
            ...
            <input
              className="in"
              value={searchInput}
              placeholder="Search Stock Allotment..."
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        ...
                        title="Download PDF"
                        onClick={() =>
                          stockAllotmentService.printDocument('/inventory/allotment/stock-allotment', row.id, 'download')
                        }
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Print"
                        onClick={() =>
                          stockAllotmentService.printDocument('/inventory/allotment/stock-allotment', row.id, 'print')
                        }
                      >
                ...
                <tr>
                  <td colSpan={COLUMNS.length + 1}>
                    <div className="empty">
                      ...
                      No records found. Click "Add Stock Allotment".
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmActionModal
        ...
        body="The stock allotment will be permanently removed from the database."
        okLabel="Delete"
        danger
