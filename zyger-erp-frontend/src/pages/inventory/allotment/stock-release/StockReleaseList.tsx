...
import type { StockReleaseListRowDto } from '../../../../types/inventory/allotment.types';
...
const COLUMNS: ColumnConfig[] = [
  ...
  { label: 'Allotment', field: 'allotmentNo' },
  { label: 'Reason', field: 'reason' },
  { label: 'Qty', field: 'qty', numeric: true },
  { label: 'Status', field: 'status' },
];

interface StockReleaseListProps {
  onAdd: () => void;
  onEdit: (id: string) => void;
  onView: (id: string) => void;
}

export default function StockReleaseList({
  onAdd,
  onEdit,
  onView,
}: StockReleaseListProps) {
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<StockReleaseListRowDto | null>(null);

  const { removeMutation } = useStockReleaseMutations();
  ...
  const handleExport = async (format: AllotmentExportFormat) => {
    try {
      await stockReleaseService.exportFile(
        'Stock Release',
    ...
    return (
      <div className="panel">
        <div className="empty">
          ...
          {getApiErrorMessage(error, 'Unable to load Stock Release records.')}
  ...
  return (
    <>
        ...
        <div className="toolbar" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
          <div className="searchwrap" style={{ minWidth: '220px', flex: '0 1 260px' }}>
            ...
            <input
              className="in"
              value={searchInput}
              placeholder="Search Stock Release..."
                  ...
                  <tr key={row.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      <button
                        ...
                        title="Download PDF"
                        onClick={() =>
                          stockReleaseService.printDocument('/inventory/allotment/stock-release', row.id, 'download')
                        }
                      >
                        <span className="material-symbols-rounded">download</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Print"
                        onClick={() =>
                          stockReleaseService.printDocument('/inventory/allotment/stock-release', row.id, 'print')
                        }
                      >
                ...
                <tr>
                  <td colSpan={COLUMNS.length + 1}>
                    <div className="empty">
                      ...
                      No records found. Click "Add Stock Release".
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmActionModal
        ...
        body="The stock release will be permanently removed from the database."
        okLabel="Delete"
        danger
